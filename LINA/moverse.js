document.addEventListener("DOMContentLoaded", () => {
  ...
  function inicializarEventos() {
    ...
    if (btnSaveCsv)
      btnSaveCsv.addEventListener("click", () => exportarMatriz("csv"));
    if (btnSaveTxt)
      btnSaveTxt.addEventListener("click", () => exportarMatriz("txt"));
  }
  ...
  function limpiarMatrizCompleta() {
    for (let i = 0; i < tabla.rows.length; i++) {
      for (let j = 0; j < tabla.rows[i].cells.length; j++) {
        ...
        const celdaInput = celda.querySelector("input");

        if (celdaInput) {
          celdaInput.value = "";
          celdaInput.style.display = "block";
        }

        const fraccionVisual = celda.querySelector(".fraccion");
        if (fraccionVisual) {
          fraccionVisual.remove();
        }
      }
    }
  }
  ...
  function reiniciarMatriz() {
    ...
    tabla.innerHTML = `
            <tr>
                <td><input type="text" class="elemento-matriz" placeholder="1" inputmode="decimal" autocorrect="off" autocapitalize="none" spellcheck="false"></td>
                <td><input type="text" class="elemento-matriz" placeholder="0" inputmode="decimal" autocorrect="off" autocapitalize="none" spellcheck="false"></td>
            </tr>
            <tr>
                <td><input type="text" class="elemento-matriz" placeholder="0" inputmode="decimal" autocorrect="off" autocapitalize="none" spellcheck="false"></td>
                <td><input type="text" class="elemento-matriz" placeholder="1" inputmode="decimal" autocorrect="off" autocapitalize="none" spellcheck="false"></td>
  ...
  function acutalizarAumento() {
    ...
    const esSEL = radioSelect.value === "sel";
...
function exportarMatriz(formato) {
  const tabla = document.getElementById("tablaMatriz");
  const filas = tabla.rows;
  let contenido = "";
  ...
  for (let i = 0; i < filas.length; i++) {
    ...
    for (let j = 0; j < filas[i].cells.length; j++) {
      const input = filas[i].cells[j].querySelector("input");

      let valor = input && input.value.trim() !== "" ? input.value.trim() : "0";
      filaData.push(valor);
    }

    let separador = formato === "csv" ? "," : "\t";
    contenido += filaData.join(separador) + "\n";
  }
  ...
  enlace.href = url;
  enlace.setAttribute("download", `matriz_escom.${formato}`);

class Fraccion {
    ...
    sumar(otra) {
        return new Fraccion(this.num * otra.den + otra.num * this.den, this.den * otra.den);
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    botonR.addEventListener("click", () => {
        ...
        intercambios = 0;
        const opcion = document.querySelector('input[name="operacion"]:checked').value;
        ...
        const matrizResuelta = GaussJordan(matriz);
        console.log("MATRIZ RESUELTA:");
        console.table(matrizResuelta.map(fila => fila.map(f => f.pasarString())));
        ...
        function matrizError() {
            ...
            for (let i = 0; i < filas; i++) {
                for (let j = 0; j < columnas; j++) {
                    const valor = tabla.rows[i].cells[j].firstChild.value.trim();
                    if (valor === "") {
                        tieneVacios = true;
                        break;
                    } else {
        ...
        function cargarMatriz () {
            ...
            for (let i = 0; i < tabla.rows.length; i++) {
                ...
                for (let j = 0; j < celdas.length; j++) {
                    const valor = celdas[j].firstChild.value.trim();
        ...
        function pasarAFraccion(valor) {
            ...
            if (valor.includes('/')) {
                ...
                if (isNaN(numerador) || isNaN(denominador) || denominador === 0) return new Fraccion(NaN, NaN);
        ...
        function interpretacion(matrizResuelta, opcion) {
            ...
            if (opcion === "sel") {
                ...
                if (contradiccion) {mensaje = "> El SEL <strong>no tiene solución</strong>";} 
                else if (filaDeCeros || y < (x - 1)) {mensaje = "> El SEL es <strong>L.D.</strong> (Infinitas soluciones).";} 
                else {mensaje = "> El SEL es <strong>L.I.</strong> (Solución única).";}
            } 
            else if (opcion === "inv") {
                ...
                if (tieneFilaCeros) {
                    htmlProceso += " * |M<sub>final</sub>| = <strong>0</strong>";
                } else {
                    console.log("Determinante diferente de 1")
                    let total = new Fraccion(intercambios % 2 === 0 ? 1 : -1, 1);
                    factoresDeterminante.forEach(f => {
                        total = total.multiplicar(pasarAFraccion(f));
                    });
                    htmlProceso += `= <strong>${total.pasarString()}</strong>`;
                }
    ...
    function moverPivote(matriz, j) {
        ...
        const a_valor = (f) => Math.abs(f.num / f.den);
    ...
    function mostrarResultado(matriz, opcion) {
        ...
        for (let i = 0; i < filas; i++) {
            ...
            for (let j = 0; j < columnas; j++) {
                ...
                } else if (opcion === "inv" && j === (columnas / 2) - 1) {
                    celdaTD.classList.add("columna-aumentada");
                } else if (opcion === "det") {
                    if (j === 0) {celdaTD.classList.add("columna-det-izquierda")}
                    if (j === columnas - 1) {celdaTD.classList.add("columna-det-derecha")};
                }

                divContenido.textContent = matriz[i][j].pasarString();
